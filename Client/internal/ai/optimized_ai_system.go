...
func (system *OptimizedAISystem) PerformPenetrationTest(target string, testType string) error {
	...
	// 执行漏洞利用阶段
	exploitResults, err := system.performExploitation(target, vulnResults)
	if err != nil {
	...
	// 生成测试报告
	report, err := system.generateReport(target, infoResults, vulnResults, exploitResults)
	if err != nil {
...
func (system *OptimizedAISystem) generateReport(target string, infoResults, vulnResults, exploitResults map[string]string) (string, error) {
...
5. 技术细节`,
		...
		if err != nil {
			system.ErrorHandler.Log("warning", "report_generation",
				"AI报告生成失败，使用默认报告",
				map[string]interface{}{"error": err.Error()})
			return system.generateDefaultReport(target, infoResults, vulnResults, exploitResults), nil
		}
...
func (system *OptimizedAISystem) generateDefaultReport(target string, infoResults, vulnResults, exploitResults map[string]string) string {
...
详细信息请参考各工具的输出结果。`,
		...
		len(infoResults), len(vulnResults), len(exploitResults))
